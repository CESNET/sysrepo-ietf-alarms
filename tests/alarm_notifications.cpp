...
TEST_CASE("Receiving alarm notifications")
{
    ...
    SECTION("Alarm status changes (/ietf-alarms:alarms/alarm-notification)")
    {
        ...
        CLIENT_ALARM_INVENTORY(cli1Sess, "alarms-test:alarm-1", "high", "true", "Some description", "description", {}, ({"minor"}));
        CLIENT_ALARM_INVENTORY(cli1Sess, "alarms-test:alarm-2", "", "false", "Another description", "another description", ({"edfa", "wss"}), ({"minor", "major"}));
        ...
        REQUIRE(dataFromSysrepo(*userSess, "/ietf-alarms:alarms/alarm-inventory", sysrepo::Datastore::Operational) == std::map<std::string, std::string>{
                    ...
                    {"/alarm-type[alarm-type-id='alarms-test:alarm-1'][alarm-type-qualifier='']/severity-level[1]", "minor"},
