...
bool includesAll(const std::map<std::string, std::string>& map, const PropsWithTimeTest& elements)
{
    return std::all_of(elements.begin(), elements.end(), [&map](const auto& e) {
        auto it = map.find(e.first);
        return it != map.end() && it->second == e.second;
    });
}
