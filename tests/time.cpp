...
TEST_CASE("Time manipulation")
{
    ...
    static const auto palindromicTimeYangString = "2022-02-22T21:20:22.020000000-00:00";

    SECTION("date::format doesn't convert to local TZ")
    {
        REQUIRE(date::format("%Y-%m-%d %H:%M:%S %z", palindromicTime) == "2022-02-22 21:20:22.020000000 +0000");
    }

    SECTION("Converts to yang format in UTC TZ")
    {
        REQUIRE(alarms::utils::yangTimeFormat(palindromicTime) == palindromicTimeYangString);
    }

    SECTION("Loads date from yang:date-and-time")
    {
