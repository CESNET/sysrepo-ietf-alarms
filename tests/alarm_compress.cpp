...
TEST_CASE("Compress alarms RPC")
{
    ...
    SECTION("Compress all")
    {
        ...
        REQUIRE(statusChanges(*userSess, alarmListInstances) == std::map<alarms::InstanceKey, std::set<StatusChangeAnyTimeBetween>>{
                    {{{"alarms-test:alarm-1", ""}, "edfa"}, {StatusChangeAnyTimeBetween{.time = edfaAlarm1[1], .perceivedSeverity = CLEARED, .text = "A warning"}}},
                    {{{"alarms-test:alarm-2", ""}, "edfa"}, {StatusChangeAnyTimeBetween{.time = edfaAlarm2[1], .perceivedSeverity = CLEARED, .text = "A warning"}}},
                    {{{"alarms-test:alarm-1", ""}, "fans"}, {StatusChangeAnyTimeBetween{.time = fansAlarm1[1], .perceivedSeverity = CRITICAL, .text = "temperature high"}}},
                });
        CLIENT_COMPRESS_RPC(userSess, 0, {});
        ...
        REQUIRE(statusChanges(*userSess, shelvedAlarmListInstances) == std::map<alarms::InstanceKey, std::set<StatusChangeAnyTimeBetween>>{
                    {{{"alarms-test:alarm-1", ""}, "wss"}, {StatusChangeAnyTimeBetween{.time = wssAlarm1[0], .perceivedSeverity = WARNING, .text = "shelf"}}},
                    {{{"alarms-test:alarm-2", ""}, "wss"}, {StatusChangeAnyTimeBetween{.time = wssAlarm2[1], .perceivedSeverity = CLEARED, .text = "shelf"}}},
                });
    ...
    SECTION("Compress by resource")
    {
        ...
        REQUIRE(statusChanges(*userSess, alarmListInstances) == std::map<alarms::InstanceKey, std::set<StatusChangeAnyTimeBetween>>{
                    {{{"alarms-test:alarm-1", ""}, "edfa"}, {StatusChangeAnyTimeBetween{.time = edfaAlarm1[1], .perceivedSeverity = 1, .text = "A warning"}}},
                    {{{"alarms-test:alarm-2", ""}, "edfa"}, {StatusChangeAnyTimeBetween{.time = edfaAlarm2[1], .perceivedSeverity = 1, .text = "A warning"}}},
                    {{{"alarms-test:alarm-1", ""}, "fans"}, {
                        StatusChangeAnyTimeBetween{.time = fansAlarm1[0], .perceivedSeverity = WARNING, .text = "temperature high"},
                        StatusChangeAnyTimeBetween{.time = fansAlarm1[1], .perceivedSeverity = CRITICAL, .text = "temperature high"},
                    }},
