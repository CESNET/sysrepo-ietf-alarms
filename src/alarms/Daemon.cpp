...
sysrepo::ErrorCode Daemon::submitAlarm(sysrepo::Session rpcSession, const libyang::DataNode& input)
{
    ...
    if (res.changed) {
        ...
        edit.newPath(alarmNodePath + "/last-raised", libyang::yangTimeFormat(it->second.lastRaised, libyang::TimezoneInterpretation::Local));
        edit.newPath(alarmNodePath + "/last-changed", libyang::yangTimeFormat(it->second.lastChanged, libyang::TimezoneInterpretation::Local));
        ...
        if (it->second.shelf) {
            ...
            edit.newPath(alarmNodePath + "/time-created", libyang::yangTimeFormat(it->second.created, libyang::TimezoneInterpretation::Local));
        }
...
void createAlarmNodeFromExistingNode(libyang::DataNode& edit, const libyang::DataNode& alarm, const InstanceKey& alarmKey, const std::chrono::time_point<std::chrono::system_clock>& now)
{
    ...
    edit.newPath(key + "/time-created", libyang::yangTimeFormat(now, libyang::TimezoneInterpretation::Local));
    createCommonAlarmNodeProps(edit, alarm, key);
}
}
...
void Daemon::updateStatistics(libyang::DataNode& edit)
{
    ...
    edit.newPath(alarmList + "/last-changed", libyang::yangTimeFormat(m_alarmListLastChanged, libyang::TimezoneInterpretation::Local));
    edit.newPath(shelvedAlarmList + "/number-of-shelved-alarms", std::to_string(totalShelved));
    edit.newPath(shelvedAlarmList + "/shelved-alarms-last-changed", libyang::yangTimeFormat(m_shelfListLastChanged, libyang::TimezoneInterpretation::Local));
}
}
