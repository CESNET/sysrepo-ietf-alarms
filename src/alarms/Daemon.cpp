...
void updateStatusChangeList(alarms::Log log, const libyang::Context& ctx, libyang::DataNode& edit, const std::string& alarmNodePath, AlarmEntry& alarm, const std::vector<TimePoint>& removedStatusChanges)
{
    ...
    auto node = *ctx.newPath2(alarmNodePath + "/status-change[time='" + time + "']", std::nullopt).createdNode;
    node.newMeta(modYang, "insert", "first");
    node.newPath(alarmNodePath + "/status-change[time='" + time + "']/perceived-severity", Severities[alarm.isCleared ? ClearedSeverity : alarm.lastSeverity]);
    node.newPath(alarmNodePath + "/status-change[time='" + time + "']/alarm-text", alarm.text);

    auto alarmNode = edit.findPath(alarmNodePath);
    auto col = alarmNode->immediateChildren();
    auto firstChange = std::find_if(col.begin(), col.end(), [](const auto& child) {
            return child.schema().name() == "status-change" && child.schema().module().name() == "ietf-alarms";
    });
    if (firstChange != col.end()) {
        firstChange->insertBefore(node);
    } else {
        alarmNode->insertChild(node);
    }

    for (const auto& time: removedStatusChanges) {
        auto xpath = alarmNodePath + "/status-change[time='" + yangTimeFormat(time) + "']";
        log->trace("Processing alarm history removal from operational DS: {}", xpath);
        edit.findPath(xpath)->unlink();
    }
}
...
void Daemon::shrinkStatusChangesLists()
{
    ...
    utils::removeFromOperationalDS(*m_edit, discardPaths);
    m_session.editBatch(*m_edit, sysrepo::DefaultOperation::Replace);
    m_session.applyChanges();
}
...
sysrepo::ErrorCode Daemon::submitAlarm(sysrepo::Session rpcSession, const libyang::DataNode& input)
{
    ...
    if (res.changed) {
        ...
        updateStatusChangeList(m_log, rpcSession.getContext(), *m_edit, alarmNodePath, it->second, res.removedStatusChanges);
...
sysrepo::ErrorCode Daemon::purgeAlarms(const std::string& rpcPath, const libyang::DataNode& rpcInput, libyang::DataNode output)
{
    ...
    if (!toDelete.empty()) {
        ...
        updateStatistics();
        utils::removeFromOperationalDS(*m_edit, toDelete);
...
void Daemon::reshelve(sysrepo::Session running)
{
    ...
    if (change) {
        ...
        updateStatistics();
        utils::removeFromOperationalDS(*m_edit, toErase);
        m_session.editBatch(*m_edit, sysrepo::DefaultOperation::Replace);
        m_session.applyChanges();
    }
}
