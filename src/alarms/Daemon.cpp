...
std::optional<std::string> shouldBeShelved(sysrepo::Session session, const alarms::AlarmKey& key)
{
    ...
    if (auto data = session.getData("/ietf-alarms:alarms/control/alarm-shelving")) {
        ...
        for (const auto& shelfNode : shelfs) {
            if (shelfMatch(shelfNode, key)) {
                session.switchDatastore(oldDatastore);
                return alarms::utils::childValue(shelfNode, "name");
            }
        }
    }
...
sysrepo::ErrorCode Daemon::submitAlarm(sysrepo::Session rpcSession, const libyang::DataNode& input)
{
    ...
    auto shelf = m_shelvingEnabled ? shouldBeShelved(m_session, alarmKey) : std::nullopt;
    ...
    if (is_cleared && (!existingAlarmNode || (existingAlarmNode && utils::childValue(*existingAlarmNode, "is-cleared") == "true"))) {
        ...
        return sysrepo::ErrorCode::Ok;
    } else if (!existingAlarmNode && !shelf) {
    ...
    if (!shelf) {
        ...
        edit.newPath(alarmNodePath + "/shelf-name", shelf);
    }
...
namespace {
void shelveExistingAlarm(libyang::DataNode& edit, const libyang::DataNode& alarm, const AlarmKey& alarmKey, const std::string& shelfName)
{
    const auto key = "/ietf-alarms:alarms/shelved-alarms/shelved-alarm[alarm-type-id='"s + alarmKey.alarmTypeId + "'][alarm-type-qualifier='" + alarmKey.alarmTypeQualifier + "'][resource='" + alarmKey.resource + "']";
...
void unshelveExistingAlarm(libyang::DataNode& edit, const libyang::DataNode& alarm, const AlarmKey& alarmKey, const std::chrono::time_point<std::chrono::system_clock>& now)
{
    const auto key = "/ietf-alarms:alarms/alarm-list/alarm[alarm-type-id='"s + alarmKey.alarmTypeId + "'][alarm-type-qualifier='" + alarmKey.alarmTypeQualifier + "'][resource='" + alarmKey.resource + "']";
...
void Daemon::reshelve()
{
    ...
    for (const auto& node : data->findXPath("/ietf-alarms:alarms/alarm-list/alarm")) {
        const auto alarmKey = getKey(node);
