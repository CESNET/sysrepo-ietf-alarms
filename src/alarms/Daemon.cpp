...
void updateAlarmSummary(sysrepo::Session session)
{
    ...
    for (const auto& alarmNode : data->findXPath(alarmListInstances)) {
        ...
        alarmSummary[severity].total += 1;
        if (isCleared == "true") {
            alarmSummary[severity].cleared += 1;
        } else {
            alarmSummary[severity].notCleared += 1;
        }
    }
    ...
    for (const auto& [severity, summ] : alarmSummary) {
        ...
        edit.newPath(prefix + "/not-cleared", std::to_string(summ.notCleared));
        edit.newPath(prefix + "/cleared", std::to_string(summ.cleared));
    }

    /* spdlog::get("main")->error("{}", *edit.printStr(libyang::DataFormat::JSON, libyang::PrintFlags::WithDefaultsAll)); */
