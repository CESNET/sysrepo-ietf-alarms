...
bool matchesResource(const libyang::Set<libyang::DataNode>& resourceNodes, const alarms::AlarmKey& alarmKey)
{
    return std::any_of(resourceNodes.begin(), resourceNodes.end(), [&alarmKey](const auto& node) {
        return std::string(node.asTerm().valueStr()) == alarmKey.resource; // FIXME regexp matcher here
    });
}
...
{
    if (auto it = std::find_if(shelfs.begin(), shelfs.end(), [&key](const auto& node) { return matchesShelf(node, key); }); it != shelfs.end()) {
